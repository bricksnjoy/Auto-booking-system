...
import { runFrame, type KitchenLayout, type Run } from "@/lib/kitchen";
...
export function kitchenModel(result: EstimateResult): Solid[] {
  ...
  for (const run of layout.runs) {
    ...
    const fd0 = D;
    const fd1 = D + door;
    const handle = (e0: number, e1: number, ya: number, yb: number) =>
      add({ kind: "handle", run: R, e0, e1, d0: fd1, d1: fd1 + 0.8, y0: ya, y1: yb, front: true });
    ...
    for (const seg of run.segments) {
      ...
      if (g === "top" && dims.pelmet > 0) {
        add({ kind: "carcass", run: R, e0: seg.e0, e1: seg.e1, d0: D - t, d1: D, y0: y0 + H - cap - dims.pelmet, y1: y0 + H - cap });
      }
      ...
      seg.cabinets.forEach((c, i) => {
        ...
        if (c.kind === "doors") {
          ...
          for (let k = 0; k < c.doors; k++) {
            ...
            add({ kind: "door", run: R, e0, e1, d0: fd0, d1: fd1, y0: y0 + gap / 2, y1: y0 + H - gap / 2, front: true });
        ...
        for (let j = 0; j < k; j++) {
          ...
          add({ kind: "drawer", run: R, e0: c.e0 + gap / 2, e1: c.e1 - gap / 2, d0: fd0, d1: fd1, y0: top - slot + gap / 2, y1: top - gap / 2, front: true });
          const mid = (c.e0 + c.e1) / 2;
          handle(mid - Math.min(3, pitch / 5), mid + Math.min(3, pitch / 5), top - 2.9, top - 2.5);
    ...
    if (g === "bottom") {
      ...
      for (const p of pieces) {
        ...
        if (p.part === "worktop" && dims.worktop > 0) {
          add({ kind: "worktop", run: R, e0: p.span.e0 + 0.02, e1: p.span.e1 - 0.02, d0: 0, d1: dims.worktop_depth, y0: y0 + H, y1: wTop });
        } else if (p.part === "tile") {
          add({
            kind: "tile",
            run: R,
    ...
    for (const o of run.openings) {
      if (g === "bottom") {
        const tall = TALL.test(o.label) && !o.worktop;
        const yTop = tall
          ? Math.min(70, layout.heights.top > 0 ? layout.heights.topY0 - 0.5 : 70)
          : o.worktop
            ? y0 + H - 0.25
            : y0 + H + dims.worktop;
        add({ kind: "appliance", run: R, e0: o.e0 + 0.25, e1: o.e1 - 0.25, d0: 1, d1: D, y0: 0, y1: yTop, label: o.label });
      } else if (/window/i.test(o.label)) {
        add({ kind: "window", run: R, e0: o.e0 + 1, e1: o.e1 - 1, d0: 0, d1: 0.3, y0: y0 + 1, y1: y0 + H - 1, label: o.label });
      } else if (/hood|chimney|extract/i.test(o.label)) {
        const w = o.e1 - o.e0;
        add({ kind: "appliance", run: R, e0: o.e0 + 1, e1: o.e1 - 1, d0: 0, d1: Math.max(D, 18) + 2, y0: y0 + H - 6, y1: y0 + H, label: o.label });
        add({ kind: "appliance", run: R, e0: o.e0 + w * 0.3, e1: o.e1 - w * 0.3, d0: 0, d1: 10, y0: y0 + H, y1: y0 + H + 10 });
      }
    }
  }
  return out;
